...
- **Light(300) 사용 금지**(디스플레이 104px급 1곳 예외 검토조차 하지 말 것). 기본 조판: **본문 500**(v3.2.1 상향 — 400은 여전히 얇다는 사용자 크리틱), 라벨·메뉴·버튼 600, h3 600, h2 700, h1·display 700. **한 화면에 500/600/700 세 웨이트가 반드시 공존**하고, 인접 텍스트 동일 웨이트 나열 금지. 본문에 400 신규 사용 금지.
...
### 16.5 푸터 압축
- 세로 최대 2단: [1단] 로고+한 문장, **Explore 링크 세로 나열**(v3.2.1 사용자 지시), Contact 이메일(우측 끝에 붙이지 않고 96px 안쪽 인셋) — 한 행 그리드. [2단] 하단 바: 좌 `© 2026 Team Bomnae Helper. All rights reserved.`(Team 칩은 이 문자열에 흡수·별도 표기 폐지), 우 Privacy Policy · Terms of Service(새 탭). 대회 풀네임·주소성 텍스트 금지. **총 높이 240px 캡은 세로 나열과 양립 불가라 해제**(v3.2.1).
