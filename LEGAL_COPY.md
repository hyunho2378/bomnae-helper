# LEGAL_COPY.md — /legal/privacy · /legal/terms 전문 (v3.1)

규칙: i18n `legal` 네임스페이스로 그대로 이식. 줄표 없음. 갱신일 표기 "Last updated: 2026-07-21" / "최종 갱신일: 2026. 7. 21." 이 문서는 학생 팀 프로토타입 기준의 초안이며 법률 자문이 아니다. 실서비스 전 전문가 검토 필요(PROGRESS 준비물 등재).
...
### 3. Why we collect it / 이용 목적
EN: We use this data only to operate your reservation: seat management, pre-ordering at partner shops on your behalf, delivery coordination for Hands-Free, and contacting you about your booking.
KR: 수집한 정보는 예약 운영 목적에만 사용합니다. 좌석 관리, 제휴 상점 선주문 대행, 핸즈프리 배송 조율, 예약 관련 연락이 그 전부입니다.

### 4. Cookies / 쿠키
EN: We use a single session cookie to keep you signed in. It is httpOnly, is not readable by scripts, and is deleted when you sign out. We do not use analytics or advertising cookies.
KR: 로그인 유지를 위한 세션 쿠키 1개만 사용합니다. 이 쿠키는 httpOnly 속성으로 스크립트에서 읽을 수 없으며 로그아웃 시 삭제됩니다. 분석·광고 쿠키는 사용하지 않습니다.

### 5. Sharing and processors / 제3자 제공·처리 위탁
EN: We share the minimum necessary details with partners who fulfill your request: the chartered bus operator sees the passenger count, partner shops see the pre-order contents, and the luggage delivery partner sees the pickup and drop-off information for Hands-Free. Our infrastructure runs on cloud hosting providers for the website and the database. We never sell personal data.
KR: 요청 이행에 필요한 최소 정보만 파트너에게 제공합니다. 전세버스 운수사에는 탑승 인원, 제휴 상점에는 선주문 내용, 수하물 딜리버리 파트너에는 핸즈프리 수거·배송 정보가 전달됩니다. 웹사이트와 데이터베이스는 클라우드 호스팅에서 운영됩니다. 개인정보를 판매하지 않습니다.

### 6. Retention / 보유 기간
EN: Booking records are kept for the duration of the Solverthon program and are deleted within 30 days after the program ends, unless a longer period is required by law.
...
KR: 방침이 변경되면 본 페이지와 상단의 갱신일을 함께 수정합니다.
...
KR: 본 사이트는 2026 STATION C 글로컬 솔버톤을 위해 제작된 프로토타입입니다. 표시된 시간표, 가격, 정류장은 초안 값이며 예고 없이 변경될 수 있습니다. 본 사이트에서 실제 결제는 이루어지지 않습니다.

### 3. Reservations and cancellation / 예약과 취소
EN: A departure is confirmed when the minimum party size is met. If we cancel a departure, any amount paid through partner channels is refunded in full. Draft policy: you may cancel free of charge until 48 hours before departure.
KR: 회차는 최소 인원이 충족되면 확정됩니다. 운영 측 사유로 회차가 취소되면 제휴 채널을 통해 지불된 금액은 전액 환불됩니다. 초안 정책: 출발 48시간 전까지는 무료로 취소할 수 있습니다.

### 4. Your responsibilities / 이용자의 의무
EN: Provide accurate arrival and contact details, arrive at the meeting point on time, and follow the host's safety guidance on board.
